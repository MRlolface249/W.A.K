...
const Home: React.FC = () => {
  ...
  const [backendData, setBackendData] = useState<{ movies: Movie[] }>({
    movies: [],
  });
  const [currentBackgroundImage, setCurrentBackgroundImage] = useState<
    string | null
  >(null); // New state variable
  ...
  const showDetail = (movie: Movie) => {
    navigate(`/detail/${movie.id}`);
  };

  const handleCarouselChange = (selectedIndex: number) => {
