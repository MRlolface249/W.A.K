...
const MovieDetailsPage: React.FC = () => {
  ...
  async function findRating(
    userRatingsData: Promise<any>,
    title: string
  ): Promise<number | null> {
    const { movies }: { movies: { movieTitle: string; rating: number }[] } =
      await userRatingsData;
  ...
  useEffect(() => {
    async function processRating() {
      ...
      if (movieData && userRating == null && user) {
        ...
        const userRatingsData = fetch(
          `${import.meta.env.VITE_BASE_API_URL}/api/${user.username}/ratings`,
          {
            method: "GET",
          }
        )
            ...
            <div className="content">
              ...
              <div className="details-section">
                ...
                <div className="movie-ratings">
                  IMDb: {movieData.vote_average}/10, Popularity:{" "}
