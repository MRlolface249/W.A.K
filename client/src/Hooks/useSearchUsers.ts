 ...
 */
export const useSearchFriends = (textFieldValue: string) => {
  ...
  useEffect(() => {
    ...
    axios
      .get<User[]>(`${import.meta.env.VITE_BASE_API_URL}/users`)
      .then((users) => {
        // Filtering the user data based on the input search value and updating the state
        setSearchUsers(
          users.data.filter((user) => user.username.includes(textFieldValue))
        );
        setIsPending(false);
        setError(null);
      })
